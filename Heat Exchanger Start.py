...
def heat_exchanger_pressure_drop(m_dot_1, m_dot_2, N, N_B, Y):
    #Hot side analysis
    rho_w = constants["rho_w"]
    d_i = constants["d_i"]
    ...
    m_dot_tube = m_dot_2/N
    v_tube = m_dot_tube*4/(rho_w*np.pi*d_i**2)
    Re_tube = Re_tube_calc(m_dot_2, N)
    ...
    A_sh = d_sh/Y * (Y-d_o) * B
    v_sh = m_dot_1 / (rho_w*A_sh)
    Re_sh = Re_sh_calc(m_dot_1, N_B, Y)                  
...
def find_flow_rates(m_dot_1, m_dot_2, N, N_B, Y, tol=1e-3, max_iter=1000):
    rho_w = constants["rho_w"]

    for i in range(max_iter):
        delta_p_1, delta_p_2 = heat_exchanger_pressure_drop(m_dot_1, m_dot_2, N, N_B, Y)
...
#Thermal analysis
def Thermal_analysis(Re_tube, Re_sh, N):
    Pr = constants["Pr"]
    k_w = constants["k_w"]
    d_i = constants["d_i"]
    d_o = constants["d_o"]
...
def search(x):
    ...
    m_dot_1_adj, m_dot_2_adj, delta_p_1, delta_p_2, pressure_cold, pressure_hot = find_flow_rates(m_dot_1_init, m_dot_2_init, int(N), int(N_B), Y)
    H, A = Thermal_analysis(Re_tube_calc(m_dot_2_adj, int(N)), Re_sh_calc(m_dot_1_adj, int(N_B), Y), N)
